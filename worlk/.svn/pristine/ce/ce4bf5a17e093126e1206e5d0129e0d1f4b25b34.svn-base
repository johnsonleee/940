	...
	<section data-role="page" id="lobby_cqss">
		...
		<div data-role="content" class="ui-mt-10">
			<div data-role="listview" class="reset ui-listview-padding">
				...
				<li class="ui-mt-10"><a href="${ctx}/jxsyxw/topage?pageName=dm&gameId=JXSYXW&betOn=BALL_1_5" title="${msg['title.ssc.DM']}" data-transition="slide" data-ajax="false"><span id="enterPlay">${msg['title.ssc.DM']}</span></a></li>
				</c:if>
				<c:if test = "${empty obj.cf || obj.cf.jxsyxw_renxuan == 1}">
				<li class="ui-mt-10"><a href="${ctx}/jxsyxw/topage?pageName=rx&gameId=JXSYXW&betOn=SERIAL_1_1&nav=1" title="${msg['title.ssc.RX']}" data-transition="slide" data-ajax="false">${msg['title.ssc.RX']}</a></li>
				</c:if>
