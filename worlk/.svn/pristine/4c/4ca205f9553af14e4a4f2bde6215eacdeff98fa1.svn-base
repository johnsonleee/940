...
<div data-role="page" id="main">
	<div data-role="header" data-theme="b">
		...
		<h2>体育{[{5oqV}]}注信息</h2>
		<a href="javascript:findSportinfo();" data-ajax="false" data-icon="refresh" class="ui-btn-right">查询</a>
	</div>
...
function findSportinfo() {
	showLoader("加载中...");
    var detailTable = $("#infoTable");
    $.ajax({
        url: '${ctx}/sys/findSportinfo',
        data: {"t" : new Date().getTime(), "yy":$("#yy").val(), "mm":$("#mm").val(), "dd":$("#dd").val(), "sum":$("#sum").val()},
        dataType: 'json',
            	...
            	if("true" == $("#sum").val()){
	                ...
	                if (arr != undefined && arr.length > 0) { 
	                    $.each(arr, function(key, value) {
	                    	var datatr ="<tr><td>" + value.sid+ "</td><td>" + value.uname + "</td><td>" + value.amount + "</td><td>" + value.time + "</td><td>" + value.year+"/"+value.month+"/"+value.day + "</td></tr>";
