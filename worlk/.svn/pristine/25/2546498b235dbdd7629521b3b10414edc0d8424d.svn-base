...
/**
 * Created by ray on 15/6/20.
 */
@IocBean(singleton = false)
@Filters({@By(type = CheckSession.class, args = {"SPORT_USER_SESSION", "/timeout"}), @By(type = AppStatusCheckFilter.class)})
@At("/sport/bb")
public class BasketballAction extends CommonAction {

    private Log log = Logs.getLog(BasketballAction.class);
    ...
    public Object leagueList(String t, String mu, String h, HttpSession session) {
        ...
        if (t.equals("1")) {
            leagues = SportBBCache.inplayLeagues;
        } else if (t.equals("2")) {
            if (mu.equals("1")) {
                leagues = SportBBCache.todayMuLeagues;

            } else {
                leagues = SportBBCache.todayLeagues;
            }
        } else if (t.equals("3")) {
            if (mu.equals("1")) {
                leagues = SportBBCache.earlyMuLeagues;
            } else {
                leagues = SportBBCache.earlyLeagues;
            }
        }
        ...
        try {
            ...
            if (leagues != null) {
                ...
                for (League tnl : tnls) {
                    String name = SportBBCache.i18nMap.get(lng + "-" + tnl.getId());
                    tnl.setName(name);
                    tlist.add(name);
                    tmap.put(name, tnl);
                }
    ...
    public Object toMatchListPage(String lid) {
        ...
        map.put("n", SportBBCache.i18nMap.get(lng + "-" + lid));
        map.put("lid", lid);
        return map;
    }
        ...
        try {
            ...
            if (league != null) {
                ...
                for (int i = 0; i < matchs.size(); i++) {
                    ...
                    mo[0] = mi.getId();
                    mo[1] = SportBBCache.i18nMap.get(lng + "-" + mi.getId());
                    Calendar c = Calendar.getInstance();
                    c.setTime(mi.getStart());
                    c.add(Calendar.HOUR_OF_DAY, 8);
                    if (t.equals("2")) {
                        mo[2] = SportBBCache.inplaySDF.format(c.getTime());
                    } else {
                        mo[2] = SportBBCache.normalSDF.format(c.getTime());
                    }
//                    mo[3] = mi.getSroll().equals("1") ? Mvcs.getMessage(req, "label.sport.fb.live") : "";
                    mo[3] = "";
                    mo[4] = mi.getGames().size();
                    if (t.equals("1")) {
                        MatchReal mr = SportBBCache.mrMap.get(mi.getId());
                        mo[2] = "";
                        mo[3] = "0 - 0";
                        if (mr != null) {
                            mo[2] = mr.getGt().contains("HT") ? "HT" : "";
                    ...
                    String[] ns = SportBBCache.i18nMap.get(lng + "-" + mi.getId()).toUpperCase().split(" VS ");
                    ...
                    if (g1 != null) {
                        ...
                        String i1n = SportBBCache.i18nMap.get(lng + "-" + g1.getItems().get(0).getId());
                        io1[11] = i1n;
                        ...
                        try {
                            if (mu.equals("1")) {
                                io1[3] = SportBBCache.rdMap.get(g1.getItems().get(0).getId())[1];
                            } else {
                                io1[3] = getRatio(g1, su, mt, SportBBCache.rdMap.get(g1.getItems().get(0).getId())[0]);
                            }
                        } catch (NullPointerException e) {
                            io1[3] = "0";
                            log.error(mt + "-" + g1.getItems().get(0).getId() + " no have ratio!");
                        }
                        io1[4] = getGameName(req, g1, null, null);
                        io1[5] = g1.getPid();

                        io2[1] = g1.getItems().get(1).getId();
                        String i2n = SportBBCache.i18nMap.get(lng + "-" + g1.getItems().get(1).getId());
                        io2[11] = i2n;
                        String[] i2ns = i2n.split(" ");
                        io2[2] = i2ns[i2ns.length - 1].contains("+") ? "" : i2ns[i2ns.length - 1];
                        try {
                            if (mu.equals("1")) {
                                io2[3] = SportBBCache.rdMap.get(g1.getItems().get(1).getId())[1];
                            } else {
                                io2[3] = getRatio(g1, su, mt, SportBBCache.rdMap.get(g1.getItems().get(1).getId())[0]);
                            }
                        } catch (NullPointerException e) {
                            io2[3] = "0";
                            log.error(mt + "-" + g1.getItems().get(1).getId() + " no have ratio!");
                        }
                        io2[4] = getGameName(req, g1, null, null);
                        io2[5] = g1.getPid();
                    } else {
                        io1[1] = "";
                        io1[2] = "";
                    ...
                    if (g2 != null) {
                        ...
                        io1[7] = SportBBCache.i18nMap.get(lng + "-" + g2.getItems().get(0).getId());
                        try {
                            if (mu.equals("1")) {
                                io1[8] = SportBBCache.rdMap.get(g2.getItems().get(0).getId())[1];
                            } else {
                                io1[8] = getRatio(g2, su, mt, SportBBCache.rdMap.get(g2.getItems().get(0).getId())[0]);
                            }
                        } catch (NullPointerException e) {
                            io1[8] = "0";
                            log.error(mt + "-" + g2.getItems().get(0).getId() + " no have ratio!");
                        }
                        io1[9] = getGameName(req, g2, null, null);
                        io1[10] = g2.getPid();

                        io2[6] = g2.getItems().get(1).getId();
                        io2[7] = SportBBCache.i18nMap.get(lng + "-" + g2.getItems().get(1).getId());
                        try {
                            if (mu.equals("1")) {
                                io2[8] = SportBBCache.rdMap.get(g2.getItems().get(1).getId())[1];
                            } else {
                                io2[8] = getRatio(g2, su, mt, SportBBCache.rdMap.get(g2.getItems().get(1).getId())[0]);
                            }
                        } catch (NullPointerException e) {
                            io2[8] = "0";
                            log.error(mt + "-" + g2.getItems().get(1).getId() + " no have ratio!");
                        }
                        io2[9] = getGameName(req, g2, null, null);
                        io2[10] = g2.getPid();
                    } else {
                        io1[6] = "";
                        io1[7] = "";
                    ...
                    if (t.equals("1")) {
                        try {
                            io1[12] = SportBBCache.mrMap.get(mi.getId()).getHred();
                            io2[12] = SportBBCache.mrMap.get(mi.getId()).getAred();
                        } catch (Exception e) {
                            io1[12] = 0;
                            io2[12] = 0;
                        }
    ...
    public Object toMatchDetailPage(String mid) {
        ...
        map.put("n", SportBBCache.i18nMap.get(lng + "-" + mid));
        map.put("mid", mid);
        return map;
    }
    ...
    public Object matchDetail(String t, String mu, String mt, String mid, HttpServletRequest req, HttpSession session) {
        ...
        try {
            ...
            if (t.equals("1")) {
                leagues = SportBBCache.inplayLeagues;
                lid = SportBBCache.inplayMTLMap.get(mid);
            } else if (t.equals("2")) {
                if (mu.equals("1")) {
                    leagues = SportBBCache.todayMuLeagues;
                } else {
                ...
                lid = SportBBCache.todayMTLMap.get(mid);
            } else if (t.equals("3")) {
                if (mu.equals("1")) {
                    leagues = SportBBCache.earlyMuLeagues;
                } else {
                ...
                lid = SportBBCache.earlyMTLMap.get(mid);
            }
            ...
            if (match != null) {
                Object[] mo = new Object[6];
                MatchReal mr = SportBBCache.mrMap.get(match.getId());
                mo[0] = match.getId();
                mo[1] = match.getItype();
                String[] ns = SportBBCache.i18nMap.get(lng + "-" + match.getId()).toUpperCase().split(" VS ");
                hn = ns[0];
                gn = ns[1];
                if (t.equals("1")) {
                    if (mr != null) {
                    ...
                    mo[2] = SportBBCache.i18nMap.get(lng + "-" + match.getId()) + "  " + (match.getTt().equals("1") ? Mvcs.getMessage(req, "label.sport.fb.zhongli") : "");
                }
                Calendar c = Calendar.getInstance();
                c.setTime(match.getStart());
                c.add(Calendar.HOUR_OF_DAY, 8);
                mo[3] = SportBBCache.normalSDF.format(c.getTime());
//                mo[4] = match.getSroll().equals("1") ? Mvcs.getMessage(req, "label.sport.fb.live") : "";
                mo[4] = "";
                if (t.equals("1")) {
                    if (mr != null) {
                        mo[3] = mr.getGt().contains("HT") ? "HT" : "";
                ...
                for (int i = 0; i < ngs.size(); i++) {
                    ...
                    go[1] = gi.getDr();
                    go[2] = gi.getInum();
                    go[3] = getGameName(req, gi, hn, gn);
                    List<Object[]> ios = new ArrayList<Object[]>();
                    for (Item item : gi.getItems()) {
                        Object[] io = new Object[4];
                        io[0] = item.getId();
                        io[1] = SportBBCache.i18nMap.get(lng + "-" + item.getId());
                        io[2] = go[2];
                        try {
                            if (mu.equals("1")) {
                                io[3] = SportBBCache.rdMap.get(item.getId())[1];
                            } else {
                                io[3] = getRatio(gi, su, mt, SportBBCache.rdMap.get(item.getId())[0]);
                            }
                        } catch (NullPointerException e) {
                            io[3] = "0";
    ...
    public Object getMr(String mids, HttpSession session) {
        ...
        try {
            ...
            for (String mid : ss) {
                MatchReal mr = SportBBCache.mrMap.get(mid);
                if (mr != null) {
                    mrs.add(mr);
                }
            }
    ...
    private double getRatio(Game game, SportUser su, String m, double rt) {
        ...
        //11是主队得分大小，12是客队得分大小，只转盘不降水
        if (",11,12,4,".contains(pid)) {
            return OddsUtil.oddsConversion(rt, OddsUtil.ODDS_TYPE_HK, Integer.parseInt(m), 3);
        }
        //其余的不处理
    ...
    private String getGameName(HttpServletRequest req, Game game, String hn, String gn) {
        String ps = "";
        String drs = "";
        ...
        } else if (game.getPid().equals("11")) {
            ps = (hn != null ? hn : "") + "  得分：大/小";
        } else if (game.getPid().equals("12")) {
            ps = (gn != null ? gn : "") + "  得分：大/小";
        }
        return ps + "  " + drs;
    }
    ...
    private void getMatchCount() {
        int count = 0;
        for (int i = 0; i < SportBBCache.inplayLeagues.size(); i++) {
            League l = SportBBCache.inplayLeagues.get(i);
            count += l.getMatches().size();
        }
        map.put("mic", count);
        count = 0;
        for (int i = 0; i < SportBBCache.todayLeagues.size(); i++) {
            League l = SportBBCache.todayLeagues.get(i);
            count += l.getMatches().size();
        }
        map.put("mtc", count);
        count = 0;
        for (int i = 0; i < SportBBCache.todayMuLeagues.size(); i++) {
            League l = SportBBCache.todayMuLeagues.get(i);
            count += l.getMatches().size();
        }
        map.put("mtmc", count);
        count = 0;
        for (int i = 0; i < SportBBCache.earlyLeagues.size(); i++) {
            League l = SportBBCache.earlyLeagues.get(i);
            count += l.getMatches().size();
        }
        map.put("mec", count);
        count = 0;
        for (int i = 0; i < SportBBCache.earlyMuLeagues.size(); i++) {
            League l = SportBBCache.earlyMuLeagues.get(i);
            count += l.getMatches().size();
        }
        map.put("memc", count);
    }
