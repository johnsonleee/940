 ...
 */
@IocBean
public class FootballFullTask implements Runnable {

	@Inject
	private PropertiesLoader propertiesLoader;

	private Log log = Logs.getLog(FootballFullTask.class);
	long count = 0;
	...
	public void run() {
		...
		try {
			...
			if (xml != null && !xml.trim().equals("")) {
				parser.parse(new ByteArrayInputStream(xml.getBytes("UTF-8")), new FootballFullHandle(false, "zh", i18nMap, inplaySport));
			}
			// 英文
			param.put("l", "en");
			...
			if (xml != null && !xml.trim().equals("")) {
				parser.parse(new ByteArrayInputStream(xml.getBytes("UTF-8")), new FootballFullHandle(true, "en", i18nMap, null));
			}
			// 其余语言...
			...
			xml = GzipUtil.unGzipString(HttpClientUtil.doGetString(propertiesLoader.getTrim("vinbet.sport.url") + "/core/apiData.getMatchInfoData.do", param, false));
			if (xml != null && !xml.trim().equals("")) {
				parser.parse(new ByteArrayInputStream(xml.getBytes("UTF-8")), new FootballFullHandle(false, "zh", i18nMap, todaySport));
			}
			// 英文
			param.put("l", "en");
			xml = GzipUtil.unGzipString(HttpClientUtil.doGetString(propertiesLoader.getTrim("vinbet.sport.url") + "/core/apiData.getMatchInfoData.do", param, false));
			...
			xml = GzipUtil.unGzipString(HttpClientUtil.doGetString(propertiesLoader.getTrim("vinbet.sport.url") + "/core/apiData.getMatchInfoData.do", param, false));
			if (xml != null && !xml.trim().equals("")) {
				parser.parse(new ByteArrayInputStream(xml.getBytes("UTF-8")), new FootballFullHandle(false, "zh", i18nMap, earlySport));
			}
			// 英文
			param.put("l", "en");
			xml = GzipUtil.unGzipString(HttpClientUtil.doGetString(propertiesLoader.getTrim("vinbet.sport.url") + "/core/apiData.getMatchInfoData.do", param, false));
			...
			SportFBCache.rver = inplaySport.getVersion();
			SportFBCache.inplayLeagues = inplayLeagues;
			SportFBCache.inplayMatchMap = inplayMatchMap;
			SportFBCache.inplayMTLMap = inplayMTLMap;
			SportFBCache.todayLeagues = todayLeagues;
			SportFBCache.todayMatchMap = todayMatchMap;
			SportFBCache.todayMuLeagues = todayMuLeagues;
			SportFBCache.todayMuMatchMap = todayMuMatchMap;
			SportFBCache.todayMTLMap = todayMTLMap;
			SportFBCache.earlyLeagues = earlyLeagues;
			SportFBCache.earlyMatchMap = earlyMatchMap;
			SportFBCache.earlyMuLeagues = earlyMuLeagues;
			SportFBCache.earlyMuMatchMap = earlyMuMatchMap;
			SportFBCache.earlyMTLMap = earlyMTLMap;
		} catch (Exception e) {
			log.error(" >> 拉取体育-足球盘口数据异常，原因：体育后台维护中或接口数据结构有调整...");
			log.debug("Sport FootballFullTask handle error:", e);
		}
		// 执行10次打一次日志
		if (count % 10 == 0) {
			log.info("Sport FootballFullTask count:" + count + " handle time:" + (System.currentTimeMillis() - start));
		}
		count++;
	}
}
