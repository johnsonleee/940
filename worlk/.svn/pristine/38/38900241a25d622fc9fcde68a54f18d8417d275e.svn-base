...
<body>
<c:set var="title" value="${msg['title.hkc.game36']}" />
<c:set var="page_title" value="${msg['title.hkc.game36']}" scope="request"/>
<div data-role="page" id="rateInfoPage">
    ...
    <div id="testt"></div>
        ...
        <div data-role="navbar" pid="36" data-mini="true">
            <%-- <ul>
    ...
    $(function () {
      ...
      //注单页面获取用户信息
        ...
        getElm("orderBtn").on("click",function(){
        	...
        	$("#hcid").text(gn);
        	gameCode = $("#selectGame option:selected").val()
        	if(tcode == '01' && $("input:checked").length < gmmk[gameCode]){// <!%-- 复式 --%>
