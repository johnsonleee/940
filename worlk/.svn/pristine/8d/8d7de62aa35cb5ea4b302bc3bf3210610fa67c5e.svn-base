...
$(function(){
	...
	$("#gameId,#status").change(function(){
		summaryMsg.html("{[{5rOo}]}{[{5Y2V}]}数：加载{[{5Lit}]}...&nbsp;&nbsp;下{[{5rOo}]}{[{6YeR}]}额：加载{[{5Lit}]}...").css("text-align","left");
		currentPage = 1;
		index = 1;	
		tbody.empty();
		$(pullUpEl).show();
...
function getBetInfo(isRefresh){
    $.ajax({
        ...
        success: function(jsonResult) {
            ...
            if (result == 0) {
                ...
                if (arr != undefined && arr.length > 0) {
                    ...
                    $.each(arr,
                    function(key, value) {
                    	var ballCxt = replaceAllHtml(value.ball);
                    	if(undefined != value.bid ){
                    		ballCxt = '<a href="${ctx}/hkc/orderGroup?flag=0&id=' + value.id + '&bid=' + value.bid + '" data-ajax="false" style="text-decoration: none;">'+formatString(value.ball, size)+'</a>';
                    	}else{
                        ...
                        var content ='<tr><th>'+ index++ +'</th> <td><font color="green"><a style="text-decoration: none;" href="javascript:openOrderIdPopup(\'' + value.id + '\')" id="'+value.id+'">'+value.gn+'</a></font></td><td>'+value.issue+'</td><td>'+value.bt+'</td><td>'+ballCxt+'<td>' + rate + '</a></td><td>' + formNumber(value.amt,2) + '</td><td>'+value.com+'&nbsp;[' + value.hdcp + '盘]</td><td>' + formNumber(value.cmny,2) + '</td></tr>';
                        content = replaceAllHtml(content);
                        tbody.append(content);
                    });
                    var summaryContent = "{[{5rOo}]}{[{5Y2V}]}数:" + jsonResult.data.bc + "&nbsp;下{[{5rOo}]}{[{6YeR}]}额:" + formNumber(Math.round(jsonResult.data.samt * 1000 )/ 1000,2)+"&nbsp;{[{5L2j}]}{[{6YeR}]}:"+formNumber(Math.round(jsonResult.data.com),2)+"&nbsp;可{[{6LWi}]}{[{6YeR}]}额:" + formNumber(Math.round(jsonResult.data.spl),2);
        ...
        error: function(jqXHR, textStatus, errorThrown) {
			if(textStatus == 'error'){
				betInfoTable.hide();summaryMsg.html('网络{[{5Lit}]}断,请检测网络').css("text-align","center").show();$(pullUpEl).hide();
...
<!-- 	 		<a href="javascript:getBetInfo(true);" data-ajax="false" class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-refresh ui-btn-icon-notext"></a> -->
  ...
  <div data-role="main" class="ui-content">
	<div class="ui-grid-a">
		<div class="ui-block-a ">
			<select name="status" id="status" data-mini="true">
				...
				<option value="1">成功{[{5rOo}]}{[{5Y2V}]}</option>
				<option value="0">失败{[{5rOo}]}{[{5Y2V}]}</option>
			</select>
		</div>
	...
	<h5 class="ui-bar ui-bar-b ui-corner-all content-info1" id="summaryMsg" style="font-size: 12px">
  	{[{5rOo}]}{[{5Y2V}]}数：加载{[{5Lit}]}...&nbsp;下{[{5rOo}]}{[{6YeR}]}额：加载{[{5Lit}]}...
  	</h5>
...
<!-- 		<table data-role="table" id="betInfoTable" data-mode="reflow" class="movie-list table-stroke" style="font-size: 12px"> -->
		  <table data-role="table" id="betInfoTable" data-mode="reflow" class="hkc-list hkc-table-breakpoint" style="font-size: 13px">
			<thead>
				<tr>
					<th>序{[{5Y+3}]}</th>
					<th>类型</th>
			...
			<tr>
				...
				<td><font color="green"><a style="text-decoration: none;"
					...
					id="fa92954d708d494b80ee854fa41f3ea5">二全{[{5Lit}]}</a></font></td>
				<td>2014122</td>
				<td>2014-10-23 16:03:25</td>
				<td><a href="${ctx}/hkc/orderGroup" data-ajax="false" style="text-decoration: none;">12,15,12,12,12,12,12,12,12,12</a>
