...
public class GameYnssAction {
	private Log log = Logs.getLog(GameYnssAction.class);

	@At("/ssc/ynssmenu")
	@Ok("jsp:page.ssc.ynss.gameYnss")
	...
	public View topage(@Param("pageName") String pageName, @Param("gameId") String gameId, @Param("betOn") String betOn, @Param("nav") String nav) {
		...
		return new ViewWrapper(new JspView("page.ssc.ynss." + pageName), dataMap);
	}
}
