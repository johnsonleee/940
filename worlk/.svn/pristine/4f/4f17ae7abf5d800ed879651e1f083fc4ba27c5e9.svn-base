...
$(function(){
   ...
   //游戏是否隐藏
    if("${obj.cf.jxsyxw_1}" != '' && "${obj.cf.jxsyxw_1}" != 1){
	   	$.each($("[id^='BALL_1-']"), function(){
	   		$(this).parent().parent().hide();
	   	});    
    }
    if("${obj.cf.jxsyxw_2}" != '' && "${obj.cf.jxsyxw_2}" != 1){
	   	$.each($("[id^='BALL_2-']"), function(){
	   		$(this).parent().parent().hide();
	   	});    
    }
    if("${obj.cf.jxsyxw_3}" != '' && "${obj.cf.jxsyxw_3}" != 1){
	   	$.each($("[id^='BALL_3-']"), function(){
	   		$(this).parent().parent().hide();
	   	});    
    }    
    if("${obj.cf.jxsyxw_4}" != '' && "${obj.cf.jxsyxw_4}" != 1){
	   	$.each($("[id^='BALL_4-']"), function(){
	   		$(this).parent().parent().hide();
	   	});    
    } 
    if("${obj.cf.jxsyxw_5}" != '' && "${obj.cf.jxsyxw_5}" != 1){
...
<body>
	<section  data-role="page" id="rateInfoPage" data-eventbind="N">
		...
		<div class="ui-on-lottery">	
			...
			<c:if test = "${empty obj.cf || obj.cf.jxsyxw_zh == 1}">
			<table border="1" class="hdptable">
				<tr class="tr-bgcolor">
			      <th data-priority="1" colspan="6">${msg['label.ssc.play.TOTAL']}</th>
			    </tr>
				<tbody>
			    <tr>
