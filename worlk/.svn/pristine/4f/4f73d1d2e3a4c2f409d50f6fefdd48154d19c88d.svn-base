...
public class GameShssAction {
	private Log log = Logs.getLog(GameShssAction.class);

	@At("/ssc/shssmenu")
	@Ok("jsp:page.ssc.shss.gameShss")
	public Object jxssmenu() {
	...
	public View topage(@Param("pageName") String pageName, @Param("gameId") String gameId, @Param("betOn") String betOn, @Param("nav") String nav) {
		...
		return new ViewWrapper(new JspView("page.ssc.shss." + pageName), dataMap);
	}
}
