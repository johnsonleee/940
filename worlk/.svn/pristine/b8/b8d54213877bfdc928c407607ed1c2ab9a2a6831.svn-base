...
<body>
<c:set var="title" value="{[{5q2j}]}{[{6IKW}]}" />
<div data-role="page" id="choosePage">
    ...
    <div data-role="main" class="ui-content">
		...
		<div data-role="navbar">
		    <ul>
		        <li><a href="#" id="gc_34" data-ajax="false" onclick="changeGcode('34', '{[{5q2j}]}{[{6IKW}]}')">{[{5q2j}]}{[{6IKW}]}</a></li>
		        <li><a href="#" id="gc_37" data-ajax="false" onclick="changeGcode('37', '{[{5q2j}]}{[{6IKW}]}{[{5LiN5Lit}]}')">{[{5q2j}]}{[{6IKW}]}{[{5LiN5Lit}]}</a></li>
		    </ul>
		</div>
                ...
                if (isAll || (!isAll && cv != o.v)) {
                    if (o.r != undefined && o.r != null) {
                        $.each(o.r, function (k, v) {
                        	if(v[2] == 0){
                        		$("[for='" + eval(k) + "']").text('封单').css('color','red');
								$("#" + eval(k)).checkboxradio("disable");
								fd = 0;
                        	}else{
                        		fd = 1;
                        		$("[for='" + eval(k) + "']").css('color','#000000');
								if(o.s==1){
