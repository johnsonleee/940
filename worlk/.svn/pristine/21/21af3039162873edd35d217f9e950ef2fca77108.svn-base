...
function changeBetOn(betOn,flag){
	$("#bon").val(betOn);
	if(betOn=='YIZI_QS'){
		$("#play_text").val(replaceAllHtml("${msg['title.ssc.YZ']} ${msg['label.ssc.play.QS']}"));
	}
	if(betOn=='YIZI_ZS'){
		$("#play_text").val(replaceAllHtml("${msg['title.ssc.YZ']} ${msg['label.ssc.play.ZS']}"));
	}
	if(betOn=='YIZI_HS'){
		$("#play_text").val(replaceAllHtml("${msg['title.ssc.YZ']} ${msg['label.ssc.play.HS']}"));
	}
	if(betOn=='YIZI_QW'){
		$("#play_text").val(replaceAllHtml("${msg['title.ssc.YZ']} ${msg['label.ssc.play.QW']}"));
	}
	if(flag==1){
...
<body>
	<section  data-role="page" id="rateInfoPage">
		...
		<div class="ui-on-lottery">
		    ...
		    <div class="handicap-info">
  			...
  			<div data-role="navbar">
			    <ul>
			        <li><a href="#" ${obj.nav == 1 ? "class='ui-btn-active'" : ""} data-ajax="false" onclick="changeBetOn('YIZI_QS')">${msg['label.ssc.play.QS']}</a></li>
			        <li><a href="#" ${obj.nav == 2 ? "class='ui-btn-active'" : ""} data-ajax="false" onclick="changeBetOn('YIZI_ZS')">${msg['label.ssc.play.ZS']}</a></li>
			        <li><a href="#" ${obj.nav == 3 ? "class='ui-btn-active'" : ""} data-ajax="false" onclick="changeBetOn('YIZI_HS')">${msg['label.ssc.play.HS']}</a></li>
			    	<li><a href="#" ${obj.nav == 4 ? "class='ui-btn-active'" : ""} data-ajax="false" onclick="changeBetOn('YIZI_QW')">${msg['label.ssc.play.QW']}</a></li>
			    </ul>
			</div>
		...
		<input type="hidden" id="play_text" value="${msg['title.ssc.YZ']} ${msg['label.ssc.play.QW']}">
	</section>
