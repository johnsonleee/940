...
<body>
	<div data-role="page" id="rateInfoPage">
		...
		<c:set var="page_title" value="三{[{5a6a}]}位" />
		...
		<div data-role="content" id="rateRefInfo">
	        ...
	        <div data-role="navbar">
			    <ul>
			    	<c:if test = "${empty obj.gcl || fn:contains(obj.gcl,'3401')}">
				    	<li><a href="#" ${obj.gameCode == 3401 ? "class='ui-btn-active'" : ""} data-ajax="false" onclick="changeGame('3401')">口口口X</a></li>
				    </c:if>
				    <c:if test = "${empty obj.gcl || fn:contains(obj.gcl,'3402')}">
				    	<li><a href="#" ${obj.gameCode == 3402 ? "class='ui-btn-active'" : ""} data-ajax="false" onclick="changeGame('3402')">口口X口</a></li>
				    </c:if>
				    <c:if test = "${empty obj.gcl || fn:contains(obj.gcl,'3403')}">
				    	<li><a href="#" ${obj.gameCode == 3403 ? "class='ui-btn-active'" : ""} data-ajax="false" onclick="changeGame('3403')">口X口口</a></li>
				    </c:if>
				    <c:if test = "${empty obj.gcl || fn:contains(obj.gcl,'3404')}">
				    	<li><a href="#" ${obj.gameCode == 3404 ? "class='ui-btn-active'" : ""} data-ajax="false" onclick="changeGame('3404')">X口口口</a></li>
			    	</c:if>
			    </ul>
			</div>
			<div data-role="navbar" id="s_ma" data-mini="true">
			    <ul>
			    	<li>
				    	<select id="s_typea" onchange="changeTT('a')">
				    		<option value="a">排序</option>
				    		<option value="b">条件</option>
				    	</select>
			    	</li>
			    	<li>
			...
			<div data-role="navbar" id="s_mb"  data-mini="true" style="display:none">
			    <ul>
			    	<li>
				    	<select id="s_typeb" onchange="changeTT('b')">
				    		<option value="a">排序</option>
				    		<option value="b">条件</option>
				    	</select>
			    	</li>
			    	<li>
				    	<select id="tj_va" onchange="changeTJ()">
			...
			<input type="hidden" id="tj_h" value="">
	        <div class="mainframe01">
	            <div class="min-overflow" id="h_div">
	                <div class="div01">
	                    ...
	                    <div class="o-number title-setting brl-1">号码</div>
	                    <div class="b-number title-setting brl-1">赔率</div>
	                </div>
	                <div class="div02">
	                    <div class="o-number title-setting brl-1">号码</div>
	                    <div class="b-number title-setting brl-1">{[{6LWU546H}]}</div>
...
function changeHandType(){
	getRate($("#gameCode").val(),$("#px_h").val(),$("#tj_h").val());
}
$(function(){
    ...
    $("#orderBtn").on("click",function(e) {
		...
		if ($("input:checked").length == 0) {
	        ...
	        $("input:checked").each(function (i) {
	            ...
	            str += "<input type='hidden' name='type' value='" + $("#t_" + $(this).attr("id")).text() + "'>";
...
function changeGame(game){
	if(game != $("#gameCode").val()){
        ...
        $("#s_ma").show();
		$("#s_mb").hide();
		$("#s_typea").val('a').selectmenu("refresh");
		$("#px_v").val(0).selectmenu("refresh");
		$("#px_h").val(0);
		$("#tj_h").val("");
		getRate($("#gameCode").val(),$("#px_h").val(),$("#tj_h").val());
	}
	if(game==3401){
		var $tj_va = $("#tj_va");
		$tj_va.empty();
		$tj_va.append('<option value="0">千位</option>');
		$tj_va.append('<option value="1">百位</option>');
		$tj_va.append('<option value="2">十位</option>');
		$tj_va.selectmenu("refresh");
	}
	if(game==3402){
		var $tj_va = $("#tj_va");
		$tj_va.empty();
		$tj_va.append('<option value="0">千位</option>');
		$tj_va.append('<option value="1">百位</option>');
		$tj_va.append('<option value="2">个位</option>');
		$tj_va.selectmenu("refresh");
	}
	if(game==3403){
		var $tj_va = $("#tj_va");
		$tj_va.empty();
		$tj_va.append('<option value="0">千位</option>');
		$tj_va.append('<option value="1">十位</option>');
		$tj_va.append('<option value="2">个位</option>');
		$tj_va.selectmenu("refresh");
	}
	if(game==3404){
		var $tj_va = $("#tj_va");
		$tj_va.empty();
		$tj_va.append('<option value="0">百位</option>');
		$tj_va.append('<option value="1">十位</option>');
		$tj_va.append('<option value="2">个位</option>');
		$tj_va.selectmenu("refresh");
	}
...
function changeTT(flag){
	var tem = $("#s_type" + flag).find("option:selected").val();
	if(tem=='a'){
		$("#s_ma").show();
		$("#s_mb").hide();
		$("#s_typea").val('a').selectmenu("refresh");
		...
		$("#s_mb").show();
		$("#s_ma").hide();
		$("#s_typeb").val('b').selectmenu("refresh");
