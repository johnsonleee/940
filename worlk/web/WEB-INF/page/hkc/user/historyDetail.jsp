                ...
                if (arr != undefined && arr.length > 0) { 
                    ...
                    function(key, value) {
                    	...
                    	if(undefined != value.bid ){
                    		ballCxt = '<a href="${ctx}/hkc/orderGroup?flag=1&id=' + value.id + '&bid=' + value.bid + '" data-ajax="false" style="text-decoration: none;">'+formatString(ballCxt, size)+'</a>';
                    	}else{
                        ...
                        var content ='<tr><th>'+ index++ +'</th> <td><font color="green"><a style="text-decoration: none;" href="javascript:openOrderIdPopup(\'' + value.id + '\')" id="'+value.id+'">'+value.gn+'</a></font></td><td>'+value.issue+'</td><td>'+value.bt+'</td><td>'+ballCxt+'<td>' + rate + '</a></td><td>' + formNumber(value.amt,2) + '</td><td>'+value.com+'&nbsp;['+value.hdcp+'盘]</td><td>' + showColor(value.win) + '</td></tr>';
                        tbody.append(replaceAllHtml(content));
                    });
                    var summaryContent = "有效{[{5rOo}]}{[{5Y2V}]}:" + jsonResult.data.bc + "&nbsp;下{[{5rOo}]}{[{6YeR}]}额:" + formNumber(Math.round(jsonResult.data.samt * 1000 )/ 1000,2)+"&nbsp;{[{5L2j}]}{[{6YeR}]}:"+formNumber(jsonResult.data.com,2)+"&nbsp;可{[{6LWi}]}{[{6YeR}]}额:" + showColor(Math.round(jsonResult.data.twin * 1000 )/ 1000,2);
...
<!-- 	 		<a href="javascript:getBetInfo(true);" data-ajax="false" class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-refresh ui-btn-icon-notext"></a> -->
  ...
  <div data-role="main" class="ui-content">
        <div style="text-align: center;">
			...
			<div class="w100 arr-center font-bold txt-12" id="anml">&nbsp;&nbsp;</div>            
        </div>  
	<h5 class="ui-bar ui-bar-b ui-corner-all content-info1" id="summaryMsg" style="font-size: 12px;height:30px;">
  	有效{[{5rOo}]}{[{5Y2V}]}：加载{[{5Lit}]}...&nbsp;可{[{6LWi}]}{[{6YeR}]}额：加载{[{5Lit}]}...
  	</h5>
	<div id="wrapper" style="top:110px;">
		<div id="scroller">
		<div id="pullDown">
			<span class="pullDownIcon"></span><span class="pullDownLabel">下拉刷新...</span>
		</div>	
<!-- 		<table data-role="table" id="betInfoTable" data-mode="reflow" class="movie-list table-stroke" style="font-size: 12px"> -->
		  <table data-role="table" id="betInfoTable" data-mode="reflow" class="hkc-list hkc-table-breakpoint" style="font-size: 12px">
			<thead>
				<tr>
					<th>序{[{5Y+3}]}</th>
					<th>类型</th>
