    ...
    return Base.extend({
        ...
        constructor: function(targetId, opt, listener) {
            ...
            if (!targetId || !document.getElementById(targetId)) {
                alert("娌℃湁鎸囧畾鏈夋晥鎾斁鍣ㄥ鍣紒");
            } else if (!opt['app_id'] || !opt['file_id']) {
                var video = opt['third_video'];
                if (video) {
                    if (!video['duration']) {
                        return alert("缂哄皯瑙嗛鏃堕暱鍙傛暟锛岃琛ラ綈duration锛�");
                    } else if (!video['urls']) {
                        return alert("缂哄皯瑙嗛鍦板潃淇℃伅锛岃琛ラ綈urls锛�");
                    } else {
    ...
    return UICom.extend({
        ...
        _click_progress_handler: function(event) {
            if (this.processDrag.isAble()) {
                ...
                this.debug('鐐瑰嚮鎾斁杩涘害鐧惧垎姣�==' + percent);
                this.fire(FIRE, {
                    event: EVENT.UI_DRAG_PLAY,
                    value: {
                        percent: percent
       ...
       <div data-resize-module="bottom" class="');
           ...
           <div style="width:100%;height:36px;margin:0px auto;position:relative;  background: #000;">\r\n\
            ...
            _p('progress-bar-container">\r\n\
                   <div class="');
            _p(this.__escapeHtml(nick));
            _p('progress-bar" aria-label="鎾斁婊戝潡" style="touch-action: none;');
    ...
    return Base.extend({
        ...
        getVideoInfo: function(res, def) {
            ...
            if (rst) {
                ...
                me.debug(':getVideoInfo result:娓呮櫚搴�==' + map['resolution'] + ';鍚嶇О==' + map['resolutionName'] + ';sdk娓呮櫚搴�' + map['resolutionNameNo']);
                return util.merge({
