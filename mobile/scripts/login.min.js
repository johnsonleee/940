...
$(".btnCheck").click(function(){$(".login-check").toggleClass("checked");});function d(){var f=$.param({code:$(".captcha").val()});$.ajax({async:false,type:"post",url:"http://940.com/checkCode",dataType:"jsonp",data:f,jsonp:"callback",success:function(g){console.log(url);
...
window.location.href="login.html";}else{if(h.status==-9){alert(h.tips);}else{alert("注册失败！");}}}});});var e=$(".login-form");e.bind("submit",function(f){f.preventDefault();
var g=$.param({user_name:$(".login-user").val(),password:$(".login-passw").val()});$.ajax({async:false,type:"get",url:"http://www.940.com/login",dataType:"jsonp",data:g,jsonp:"callback",success:function(i){console.log(g);
if(i.status==1){alert(i.tips);var h={u:$(".login-user").val(),password:$(".login-passw").val()};localStorage.member=JSON.stringify(h);window.location.href="user.html";
}else{if(i.status==-9){alert(i.tips);}else{alert("data.tips");}}}});});});